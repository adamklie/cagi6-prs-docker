...
Inputs: 1) a config path for model instantiation 2) An out-file path
...
def DoTheThing(modelPath, config, fileOutputPath = "./TestSetPredictions"):
    ...
    #instantiate an untrained model for predictions
    ...
    #testPredictions.applymap(lambda x: x[1:-1]) #remove the annoying [] around each prediction probability
...
if __name__ == "__main__":
    ...
    #path_args.add_argument("--model_path", type = str, help = "Path to the trained InSNPtion model") # --> For testing not loading in a trained model so not needed
    ...
    path_args.add_argument("--out_path", type = str, default = "/cellar/users/jtalwar/projects/BetterRiskScores/InSNPtion/Galbatorix/DeweyIsOutOfTheGAGI/OtherStuff/DummyPreds", help = "Folder path where predictions will be written.")
    ...
    DoTheThing(None, config, fileOutputPath = basePath + nameNames + ".tsv" ) 
